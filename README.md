...
**The solution:** RemembrallMCP gives agents persistent memory - decisions, patterns, code relationships, and organizational context that survives between sessions.
...
| `remembrall_ingest_docs` | Scan a project directory for markdown files and ingest them as memories |
...
| `remembrall_index` | Index a project directory (8 languages supported) |
...
cargo install --path crates/remembrall-server
...
This sets up a Docker-managed Postgres container with pgvector, creates the schema, and pre-downloads the embedding model. Config is written to `~/.remembrall/config.toml`.
...
A new RemembrallMCP instance has no knowledge. Use the ingestion tools to bootstrap from existing project history in minutes.
...
crates/
  ...
  remembrall-server/        # MCP server + CLI binary (remembrall)
