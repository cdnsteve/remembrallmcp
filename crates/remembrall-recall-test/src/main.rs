//! RemembrallMCP recall test harness.
//!
//! Validates remembrall_recall search quality against a ground truth TOML file.
//!
//! Usage:
//!   remembrall-recall-test \
//!     --database-url postgres://postgres:postgres@localhost:5450/remembrall \
//!     --seed /path/to/seed_memories.toml \
//!     --ground-truth /path/to/ground_truth.toml \
//!     [--schema remembrall_test] \
//!     [--no-seed]   # skip seeding if data is already present
//!
//! Environment variables:
//!   DATABASE_URL  - overrides --database-url if set
//!   TEST_SCHEMA   - overrides --schema if set (default: remembrall_test)
//!
//! Exit codes:
...
use remembrall_core::embed::FastEmbedder;
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            std::env::var("RUST_LOG")
                .unwrap_or_else(|_| "remembrall_recall_test=info,remembrall_core=warn".into()),
        )
        .init();
...
fn parse_args() -> Result<Args> {
    ...
    let mut database_url = String::from("postgres://postgres:postgres@localhost:5450/remembrall");
    let mut schema = String::from("remembrall_test");
