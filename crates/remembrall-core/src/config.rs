...
impl Config {
    pub fn from_env() -> Self {
        Self {
            database_url: std::env::var("REMEMBRALL_DATABASE_URL")
                .or_else(|_| std::env::var("DATABASE_URL"))
                .expect("REMEMBRALL_DATABASE_URL or DATABASE_URL must be set"),
            pool_size: std::env::var("REMEMBRALL_POOL_SIZE")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(25),
            schema: std::env::var("REMEMBRALL_SCHEMA")
