...
impl FastEmbedder {
    pub fn new() -> Result<Self> {
        ...
        .map_err(|e| {
            crate::error::RemembrallError::Internal(format!(
...
impl Embedder for FastEmbedder {
    fn embed(&self, text: &str) -> Result<Vec<f32>> {
        ...
        let results = model
            .embed(vec![text], None)
            .map_err(|e| {
                crate::error::RemembrallError::Internal(format!("Embedding failed: {e}"))
            })?;
        results.into_iter().next().ok_or_else(|| {
            crate::error::RemembrallError::Internal("No embedding returned".into())
        })
    }
    ...
    fn embed_batch(&self, texts: &[&str]) -> Result<Vec<Vec<f32>>> {
        ...
        model.embed(owned, None).map_err(|e| {
            crate::error::RemembrallError::Internal(format!("Batch embedding failed: {e}"))
        })
    }
